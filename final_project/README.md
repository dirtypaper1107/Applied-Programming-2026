...
The application connects to a TCP server, receives raw EMG-like signal data, reconstructs the incoming byte stream into NumPy arrays, and displays the data in real time. After streaming is stopped or disconnected, the recorded data can be inspected offline with Matplotlib.
...
Start the GUI application in a second terminal:
...
6. Click `Show Offline Plot` to inspect the recorded signal.
